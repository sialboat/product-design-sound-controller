...
public:
  ...
  single_pin_out_param(String n, struct device_pin p)
    : pin(p) {
    set_name(n);
  }
...
public:
  ...
  multi_pin_out_param(String n, std::initializer_list<device_pin> p_list)
    : pins(p_list) {
    set_name(n);
  }
  ~multi_pin_out_param();
  ...
  std::vector<N>* get_vals() {
    return &processed_vals;
  }
  std::vector<N>* get_raw_vals() {
    return &raw_vals;
  }
  std::vector<device_pin>* get_device_pins() {
    return &pins;
  }
  String get_pin(int i) {
    return pins.at(i).name + ": " + pins.at(i).pin;
  }
  String get_pins() {
    String out = "";
  ...
  void set_pin(int i, int v) {
    pins.at(i).pin = v;
  }
  void set_pin_name(int i, String v) {
    pins.at(i).name = v;
  }
...
public:
  button()
    : single_pin_out_param<bool>("untitled-button", { "nonexistent-pin", -1 }) {

    type = PARAM_TYPE::BUTTON;
  }
  button(String n, device_pin p)
    : single_pin_out_param<bool>(n, p) {
...
public:
  pot()
    : single_pin_out_param<int>("untitled-pot", { "nonexistent pin", -1 }) {
    type = PARAM_TYPE::POT;
  }
  pot(String n, device_pin p)
    : single_pin_out_param<int>(n, p) {
    type = PARAM_TYPE::POT;
