...
class moving_average_filter
{
  ...
  int bias_int;
  public:
    moving_average_filter(float a = 0.4f, float bias = 0) : alpha(a), smoothed_float(0.0f), smoothed_int(0), 
      initialized_float(false), initialized_int(false), bias_float(bias), bias_int(0) {}
    moving_average_filter(float a = 0.4f, int bias = 0) : alpha(a), smoothed_float(0.0f), smoothed_int(0), 
